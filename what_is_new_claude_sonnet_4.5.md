...
Starting with Claude Sonnet 4.5, we've introduced [global endpoint pricing](/en/docs/about-claude/pricing#third-party-platform-pricing) for AWS Bedrock and Google Vertex AI. These platforms now offer two endpoint types:

* **Global endpoints**: Dynamic routing for maximum availability with no pricing premium
* **Regional endpoints**: Guaranteed data routing through specific geographic regions with a **10% pricing premium**

**The Claude API (1P) is global by default and unaffected by this change** - there are no regional pricing differences when using the Claude API directly.
